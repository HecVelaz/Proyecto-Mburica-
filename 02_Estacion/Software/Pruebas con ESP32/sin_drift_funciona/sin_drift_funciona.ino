...
const long INTERVALO_SEGUNDOS = 5 * 60; // 5 minutos en segundos
...
void setup() {
  ...
  // Establecemos el próximo envío al siguiente intervalo de 5 minutos
  proximoEnvio = DateTime(ahora.year(), ahora.month(), ahora.day(), 
                         ahora.hour(), ahora.minute() + minutosParaSiguienteIntervalo, 0);
  ...
  Serial.print("Próxima lectura programada a las: ");
  sprintf(buffer, "%02d:%02d:00", proximoEnvio.hour(), proximoEnvio.minute());
  Serial.println(buffer);
...
// --- BUCLE PRINCIPAL ---
void loop() {
  // Verificamos la hora actual del RTC
  DateTime ahora = rtc.now();
  
  // ¿Ha llegado el momento del próximo envío?
  if (ahora.unixtime() >= proximoEnvio.unixtime()) {
    ...
    proximoEnvio = DateTime(proximoEnvio.unixtime() + INTERVALO_SEGUNDOS);
    
    Serial.print("Próximo envío programado para: ");
    char buffer[20];
    sprintf(buffer, "%02d:%02d:00", proximoEnvio.hour(), proximoEnvio.minute());
    Serial.println(buffer);
  }
